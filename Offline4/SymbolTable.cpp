...
public:
    ...
    {
        if(parentID != "")
            id = parentID + "_" + to_string(prevSerial+1);
        else
    ...
    {
        int i = 0;
        SymbolInfo* temp = new SymbolInfo(name, type, DataType);
        int idx = hash(name);
...
public:
    ...
    void EnterScope(ofstream &output)
    {
    ...
    void ExitScope(ofstream &output)
    {
    ...
    bool Insert(string name, string type, string DataType)
    {
        ...
        bool check = currentScope->Insert(name, type, DataType);
        if(check == false){
